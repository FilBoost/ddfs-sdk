...
import (
	...
	"ddfs-sdk/addpiece_ext/proxyreader"
	"ddfs-sdk/api"
	"ddfs-sdk/api/impl"
	"ddfs-sdk/utils"
)
...
func GenUnseal(unsealPath string, sector abi.SectorID, pieceSize abi.UnpaddedPieceSize, info DD_Addpiece_Param) (abi.PieceInfo, error) {
	beginWaitTask := time.Now()
	genUnsealTaskCount <- struct{}{}
	defer func() {
		<-genUnsealTaskCount
	}()
	log.Infof("[DD] ready to generate unseal to %v,cost:[%v]", unsealPath, time.Since(beginWaitTask).Truncate(time.Second))

	var err error
	var confirm func(string) error
	var file io.ReadCloser

	file, confirm, err = GetRemoteFile(info.RemoteFileUrl)
	if err != nil {
	...
	defer func() {
		if err == nil && confirm != nil {
			ConfirmProxy(confirm, info.RemoteFileUrl, sector, 60, time.Minute*2)
		}
	}()
...
func GetRemoteFile(remoteFileUrl string) (api.RemoteFile, func(string) error, error) {
	fOpt, err := impl.NewFileOpt(remoteFileUrl)
	if err != nil {
		return nil, nil, err
	}
...
func ConfirmFetched(remoteFileUrl string, id abi.SectorID) error {
	fileOpt, err := impl.NewFileOpt(remoteFileUrl)
	if err != nil {
		return err
	}
...
func ConfirmProxy(confirm func(string) error, remoteFileUrl string, id abi.SectorID, errLimit int, errWait time.Duration) {
	...
	log.Errorf("[DD] confirm %v failed,key: %v,err: %v,errNum: %v", remoteFileUrl, key, err, 0)
	...
	go func() {
		for i := 1; i < errLimit; {
			...
			log.Errorf("[DD] confirm %v failed,key: %v,err: %v,errNum: %v", remoteFileUrl, key, err, i)
			i++
			if i < errLimit {
...
var genUnsealTaskCount = func() (genUnsealTaskCount chan struct{}) {
	const DD_ADDPIECE_TASK_COUNT_ENV = "DD_ADDPIECE_TASK_COUNT"
	genUnsealTaskCount = make(chan struct{}, 3)
	_genUnsealTaskCount := os.Getenv(DD_ADDPIECE_TASK_COUNT_ENV)
	if _genUnsealTaskCount == "" {
		return
	}

	taskCount, err := strconv.Atoi(_genUnsealTaskCount)
	if err != nil {
		log.Errorf("[DD] env DD_ADDPIECE_TASK_COUNT err:[%v]", err)
		return
	}

	if taskCount <= 0 {
		log.Errorf("[DD] env DD_ADDPIECE_TASK_COUNT[%v] need to be bigger than 0", taskCount)
		return
	}

	genUnsealTaskCount = make(chan struct{}, taskCount)
	return
}()
